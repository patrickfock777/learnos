...
export default function YoutubePage() {
  ...
  async function summarize() {
    ...
    try {
      const res = await fetch('/api/youtube-summary', {
        ...
        body: JSON.stringify({url, language, focus})
      })
  ...
  function reset() {
    ...
    setEditTitle(''); setEditSummary(''); setError(''); setFocus('')
  }
          ...
          <div>
            <div style={{background:C.white,border:`1px solid ${C.border}`,borderRadius:'12px',padding:'1.25rem',marginBottom:'1rem'}}>
              <div style={{fontSize:'13px',color:C.text2,marginBottom:'12px',lineHeight:'1.6'}}>
                Füge eine YouTube URL ein — die KI liest die Untertitel und erstellt eine strukturierte Zusammenfassung für deine Bibliothek.
              </div>
              <input
                value={url}
              ...
              <div style={{marginBottom:'12px'}}>
                <div style={{fontSize:'11px',color:C.text2,marginBottom:'4px',fontWeight:600,textTransform:'uppercase' as const,letterSpacing:'0.05em'}}>
                  Fokus <span style={{fontWeight:400,textTransform:'none' as const}}>(optional — worauf soll sich die KI konzentrieren?)</span>
                </div>
                <textarea
                  value={focus}
                  onChange={e=>setFocus(e.target.value)}
                  placeholder={'z.B. "Fokus auf Verkaufsstrategien für DOOH und Retail Media"\noder "Nur praktische Tipps, keine Theorie"\noder "Was kann ich bei Grassfish konkret anwenden?"'}
                  rows={3}
              ...
              <button onClick={summarize} disabled={!url.trim()} style={{width:'100%',padding:'11px',background:url.trim()?C.primary:'#ccc',color:'#fff',border:'none',borderRadius:'8px',fontSize:'14px',fontWeight:600,cursor:url.trim()?'pointer':'default'}}>
                ✨ Zusammenfassen
              </button>
            </div>
